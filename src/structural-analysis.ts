...
function stripCommentsAndStrings(source:string){return source.replace(/\/\*[\s\S]*?\*\//g,m=>m.replace(/[^\n]/g," ")).replace(/\/\/.*$/gm,"").replace(/"(?:\\.|[^"\\])*"/g,'""').replace(/'(?:\\.|[^'\\])*'/g,"''");}
...
function modifiersOf(signature:string,parameterText:string){const tail=signature.slice(signature.indexOf(")")+1);const reserved=new Set(["external","public","internal","private","view","pure","payable","returns","virtual","override"]);return Array.from(new Set((tail.match(/\b[A-Za-z_][A-Za-z0-9_]*(?:\([^)]*\))?/g)??[]).map(x=>x.replace(/\(.*$/,"")).filter(x=>!reserved.has(x)&&x!==parameterText)));}
function parameterNames(text:string){return text.split(",").map(part=>part.trim().match(/(?:^|\s)([A-Za-z_][A-Za-z0-9_]*)\s*$/)?.[1]).filter((x):x is string=>Boolean(x));}
function hasAccessControl(signature:string,body:string){return /\b(?:only[A-Z][A-Za-z0-9_]*|onlyOwner|onlyAdmin|nonReentrant|whenNotPaused)\b/.test(signature)||/\b(?:require|revert|assert)\s*\([^;\n]*(?:msg\.sender|hasRole|owner|admin|authorized)\b/.test(body)||/\b(?:hasRole|_checkRole|_authorizeUpgrade)\s*\(/.test(body);}
function findStateVariables(clean:string){const variables:string[]=[];for(const m of clean.matchAll(/\b(?:uint(?:8|16|32|64|128|256)?|int(?:8|16|32|64|128|256)?|address|bool|bytes(?:32)?|mapping\s*\([^;]+\)|string)\s+(?:public|private|internal)?\s*(?:immutable|constant)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*(?:=[^;]*)?;/g))variables.push(m[1]);return Array.from(new Set(variables));}
function calledFunctions(body:string,self:string){const ignored=new Set(["if","for","while","require","revert","assert","return","emit","abi"]);const names:string[]=[];for(const m of body.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\s*\(/g)){const n=m[1];if(n!==self&&!ignored.has(n)&&!names.includes(n))names.push(n);}return names;}
export function analyzeSolidityStructure(source:string,file="unknown.sol"):StructuralFileAnalysis{
 const clean=stripCommentsAndStrings(source),contracts=Array.from(clean.matchAll(/\b(?:contract|library|interface)\s+([A-Za-z_][A-Za-z0-9_]*)/g),m=>m[1]),stateVariables=findStateVariables(clean),functions:StructuralFunction[]=[];
 const functionRe=/\bfunction\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)([^\{;]*?)\{/g;
 for(const match of clean.matchAll(functionRe)){const open=match.index!+match[0].lastIndexOf("{"),close=matchingBrace(clean,open);if(close<0)continue;const signature=match[0],body=clean.slice(open+1,close),params=parameterNames(match[2]),modifiers=modifiersOf(signature,match[2]);
  const stateWrites=Array.from(new Set(Array.from(body.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)(?:\s*\[[^\]]+\])?\s*(?:\+=|-=|\*=|\/=|%=|=)/g),m=>m[1])).filter(name=>stateVariables.includes(name)));
  const externalCalls=Array.from(new Set(Array.from(body.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\.(?:call|callcode|staticcall)\s*(?:\{|\()/g),m=>m[1])));
  const delegateCalls=Array.from(new Set(Array.from(body.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\.delegatecall\s*\(/g),m=>m[1])));
  const valueTransfers=Array.from(new Set([...Array.from(body.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\.call\s*\{\s*value\s*:/g),m=>m[1]),...Array.from(body.matchAll(/\b(?:transfer|send)\s*\(/g),()=> "native-transfer")]));
  const tokenTransfers=Array.from(new Set(Array.from(body.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\.(transfer|transferFrom|safeTransfer|safeTransferFrom|approve|safeApprove)\s*\(/g),m=>`${m[1]}.${m[2]}`)));
  const signatureOperations=Array.from(new Set(Array.from(body.matchAll(/\b(?:ecrecover|ECDSA\.(?:recover|toEthSignedMessageHash)|SignatureChecker\.|permit\s*\()/g),m=>m[0].replace(/\s+/g,""))));
  const nonceWrites=Array.from(new Set(Array.from(body.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*nonce[A-Za-z0-9_]*)\s*(?:\+\+|--|\+=|=)/gi),m=>m[1])));
  const oracleReads=Array.from(new Set(Array.from(body.matchAll(/\b([A-Za-z_][A-Za-z0-9_]*)\.(?:latestRoundData|getPrice|getAssetPrice|latestAnswer|consult|quote)\s*\(/g),m=>m[1])));
  const accountingSignals=Array.from(new Set(Array.from(body.matchAll(/\b(?:balanceOf|totalSupply|totalAssets|totalShares|debt|shares|assets|liquidity|reserve|collateral|balance)\b/gi),m=>m[0].toLowerCase())));
  const upgradeOperations=Array.from(new Set(Array.from(body.matchAll(/\b(?:upgradeTo|upgradeToAndCall|setImplementation|changeAdmin|_upgradeTo|_setImplementation)\s*\(/g),m=>m[0].replace(/\s+/g,""))));
  ...
  const externalPositions=Array.from(body.matchAll(/\b[A-Za-z_][A-Za-z0-9_]*\.(?:call|callcode|staticcall|delegatecall)\s*(?:\{|\()/g),m=>m.index??0);
  const lastExternal=externalPositions.length?Math.max(...externalPositions):-1;
  const statePositions=Array.from(body.matchAll(/\b[A-Za-z_][A-Za-z0-9_]*(?:\s*\[[^\]]+\])?\s*(?:\+=|-=|\*=|\/=|%=|=)/g),m=>m.index??0);
