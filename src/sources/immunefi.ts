...
export class ImmunefiBountySource implements BountySource {
  ...
  async discover(): Promise<BountyProgram[]> {
    const response = await fetch(ENDPOINT, { headers: { "user-agent": "ONCHAIN-HUNTER/0.2" } });
    ...
    return (data as ImmunefiRecord[]).map((item, index) => {
      const scopeValues = [
        ...(item.assetsInScope ?? []).flatMap(asset => [asset.url, asset.name].filter(Boolean) as string[]),
        ...(item.resources ?? []).flatMap(resource => [resource.url, resource.name].filter(Boolean) as string[])
      ];
      ...
      return {
        ...
        rewardCurrency: item.rewardToken,
