...
async function main(): Promise<void> {
 ...
 const bountyIntelligence=rankBountyIntelligence(bountyMatches.flatMap(x=>buildBountyIntelligence(evidencePackages.find(p=>p.packageId===x.packageId)!,x.matches,programs)));
 ...
 const orchestration=await orchestrateInvestigations(evidencePackages,investigation.current);
 const proofDossiers=evidencePackages.map(pkg=>buildProofDossier(pkg));
 const validationBundles=[];for(let i=0;i<evidencePackages.length;i++){validationBundles.push(await prepareValidationBundle(evidencePackages[i],proofDossiers[i]));}
 const workstation=buildResearchWorkstation(evidencePackages,investigation.current,orchestration,proofDossiers,validationBundles,bountyIntelligence);
