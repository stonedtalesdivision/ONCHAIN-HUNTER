...
import { orchestrateInvestigations } from "../investigation-orchestrator.js";
import { buildProofDossier } from "../proof-engine.js";
import { prepareValidationBundle } from "../validation-orchestrator.js";
import { buildBountyIntelligence, rankBountyIntelligence } from "../bounty-intelligence.js";
import { buildResearchWorkstation } from "../research-workstation.js";
import { rankExploitability } from "../exploitability-gate.js";
...
async function main(): Promise<void> {
 ...
 const exploitability=rankExploitability(evidencePackages);
 const gatedPackages=exploitability.candidates;
 const bountyMatches=gatedPackages.map(pkg=>({packageId:pkg.packageId,findingId:pkg.findingId,matches:rankBountyMatches(matchEvidencePackage(pkg,programs,payoutRoutesForProgram))}));
 const bountyIntelligence=rankBountyIntelligence(bountyMatches.flatMap(x=>buildBountyIntelligence(gatedPackages.find(p=>p.packageId===x.packageId)!,x.matches,programs)));
 ...
 const investigation=await updateInvestigationState(gatedPackages);
 const orchestration=await orchestrateInvestigations(gatedPackages,investigation.current);
 const proofDossiers=gatedPackages.map(pkg=>buildProofDossier(pkg));
 const validationBundles=[];for(let i=0;i<gatedPackages.length;i++){validationBundles.push(await prepareValidationBundle(gatedPackages[i],proofDossiers[i]));}
 const workstation=buildResearchWorkstation(gatedPackages,investigation.current,orchestration,proofDossiers,validationBundles,bountyIntelligence);
 await writeFile("artifacts/hunt/exploitability-gate.json",JSON.stringify({schemaVersion:"phase-15-gate",generatedAt:new Date().toISOString(),inputPackages:evidencePackages.length,validationCandidates:exploitability.assessments.filter(a=>a.tier==="validation").length,reviewCandidates:exploitability.assessments.filter(a=>a.tier==="review").length,deprioritized:exploitability.assessments.filter(a=>a.tier==="deprioritized").length,assessments:exploitability.assessments.sort((a,b)=>b.score-a.score)},null,2),"utf8");
 ...
 const result={generatedAt:new Date().toISOString(),source:source.name,programsDiscovered:programs.length,attemptedRepositories,scannedRepositories,rateLimited,skippedRepositories,scopeReviewRequired,targetPlan,candidateFindings:findings.length,exploitabilityGate:{inputPackages:evidencePackages.length,validationCandidates:exploitability.assessments.filter(a=>a.tier==="validation").length,reviewCandidates:exploitability.assessments.filter(a=>a.tier==="review").length,deprioritized:exploitability.assessments.filter(a=>a.tier==="deprioritized").length,artifactPath:"artifacts/hunt/exploitability-gate.json"},payoutConfiguration,structuralAnalysis:structuralSummaries,evidenceGraphs:rankedGraphs,evidenceGraphSummary:{total:evidenceGraphs.length,unique:rankedGraphs.length,crossFunctionChains:rankedGraphs.filter(g=>g.crossFunctionPaths.length).length,critical:rankedGraphs.filter(g=>g.severity==="critical").length,high:rankedGraphs.filter(g=>g.severity==="high").length},evidencePackages,evidencePackageSummary:{total:evidencePackages.length,gatedForReview:gatedPackages.length,reviewReady:evidencePackages.filter(p=>p.submissionReady).length,withCrossFunctionChains:evidencePackages.filter(p=>p.transactionSequence.length>1).length},bountyMatchSummary:{packages:evidencePackages.length,matched:bountyMatches.filter(x=>x.matches.length).length,exactScope:bountyMatches.reduce((n,x)=>n+x.matches.filter(m=>m.scope==="exact-repository").length,0)},monitoring:{events:monitoring.events.length,eventsPath:"artifacts/monitoring/latest-events.json",statePath:"artifacts/monitoring/state.json"},investigation:{total:Object.keys(investigation.current.records).length,revisionChanges:Object.values(investigation.current.records).filter((record)=>record.revisionChanged).length,artifactPath:"artifacts/investigations/latest.json"},orchestration:{selected:orchestration.selected?.packageId??null,queue:orchestration.queue.length,artifactPath:"artifacts/investigations/orchestration.json"},proof:{total:proofDossiers.length,strong:proofDossiers.filter(p=>p.proofScore>=80).length,artifactPath:"artifacts/validation-bundles"},validationBundles:{total:validationBundles.length,prepared:validationBundles.length,executionEnabled:false,artifactPath:"artifacts/validation-bundles"},bountyMatches,bountyIntelligence:{total:bountyIntelligence.length,top:bountyIntelligence.slice(0,10),artifactPath:"artifacts/hunt/bounty-intelligence.json"},researchWorkstation:{total:workstation.summary.total,blocked:workstation.summary.blocked,readyForReview:workstation.summary.readyForReview,artifactPath:"artifacts/hunt/research-workstation.json"},results:candidates};
 await writeFile(path,JSON.stringify(result,null,2),"utf8");
 await writeFile("artifacts/hunt/bounty-intelligence.json",JSON.stringify({schemaVersion:"phase-13",generatedAt:result.generatedAt,total:bountyIntelligence.length,items:bountyIntelligence},null,2),"utf8");console.log(JSON.stringify(result,null,2));
}
