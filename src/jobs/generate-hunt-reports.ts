...
const huntPath="artifacts/hunt/latest.json";
...
reports.sort((a,b)=>{const ia=investigation?.records[a.packageId]?.priority??0;const ib=investigation?.records[b.packageId]?.priority??0;return ib-ia||(severityRank[b.severity]??0)-(severityRank[a.severity]??0)||b.confidence-a.confidence;});
const reviewQueue={schemaVersion:"phase-9",generatedAt:new Date().toISOString(),submissionEnabled:false,totalReports:reports.length,priorityOrder:"investigation-priority-desc",reports:reports.map((r,index)=>({priority:index+1,investigationPriority:investigation?.records[r.packageId]?.priority??0,revisionChanged:investigation?.records[r.packageId]?.revisionChanged??false,missingEvidence:investigation?.records[r.packageId]?.missingEvidence??[],packageId:r.packageId,findingId:r.findingId,programId:r.program.id,severity:r.severity,confidence:r.confidence,scope:r.bounty.scope,validation:r.validation.status,reviewBlockers:r.reviewBlockers,submissionReady:r.submissionReady}))};
