...
const reviewQueue={schemaVersion:"phase-7",generatedAt:new Date().toISOString(),submissionEnabled:false,totalReports:reports.length,reports:reports.map(r=>({packageId:r.packageId,findingId:r.findingId,programId:r.program.id,severity:r.severity,confidence:r.confidence,scope:r.bounty.scope,validation:r.validation.status,reviewBlockers:r.reviewBlockers,submissionReady:r.submissionReady}))};
